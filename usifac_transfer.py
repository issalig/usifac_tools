...
#amsdos header http://www.cpcwiki.eu/imgs/b/bc/S968se09.pdf
def amsdos_file_info(data):
    user_number = data[0]
    file_name = data[1:12].decode("utf-8", errors='ignore') #ignore strange chars
    file_type = data[18]
    file_length= data[64]+256*data[65]+256*256*data[66]
    mychecksum=sum(data[0:66])
    checksum=data[67]+256*data[68]
    
    if (checksum != mychecksum):
        print('Not a valid AMSDOS header, checksum incorrect')
